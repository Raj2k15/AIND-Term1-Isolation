...
def custom_score(game, player):
    ...
    x = dict((a, game.get_moves(a)) for a in game.get_blank_spaces())
    l=0
...
#        return self.best_move,alpha,game.active_player
    ...
    def alpha_max_player(self,game,depth,alpha,beta):
        #Terminal check in max layer for each call
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        if depth == 0:
            return self.score(game,game.active_player)
        #intializing alpha in max layer 
        alpha = float("-inf")
        for m in game.get_legal_moves():
            updated_game = game.forecast_move(m)
            score_min = self.alpha_min_player(updated_game,depth-1,alpha,beta)
            alpha=max(score_min,alpha)
            #Alpha beta Pruning
            if alpha >= beta:
                break
    ...
    def alpha_min_player(self,game,depth,alpha,beta):
        #Terminal check in min layer for each player
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        if depth == 0:
            return self.score(game,game.inactive_player)
        #initializing beta in min layer
        beta = float("inf")
        for m in game.get_legal_moves():
            updated_game = game.forecast_move(m)
            score_max = self.alpha_max_player(updated_game,depth-1,alpha,beta)
            beta=min(score_max,beta)
            if beta<= alpha:
                break
        return beta       
